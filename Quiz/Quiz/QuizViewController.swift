...
//  Quiz
//
//  Created by spark-05 on 2024/05/07.
//
...
class QuizViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.loadQuiz()
        let panGestureReognizer =
        UIPanGestureRecognizer(target: self, action: #selector(dragQuizCard(_:)))
        self.quizCard.addGestureRecognizer(panGestureReognizer)
    }
    ...
    func answer()  {
        ...
        UIView.animate(withDuration: 0.5, delay: 0.1, options: [.curveLinear],animations: { self.quizCard.transform = translationTransform},
                       completion: {[unowned self](finished)in if finished {self.showNextQuiz()
                       }
        })
    ...
    func transformQuizCard(gesture: UIPanGestureRecognizer){
        ...
        let transform =
        translationTransform.concatenating(rotationTransform)
